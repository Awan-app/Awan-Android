...
- Deep review fix wave resolved state synchronization between header and calendar grid: `loadTodayActivity` now updates `streakDates` and rebuilds `monthDays` atomically; `observeGamificationProgressUseCase` preserves `isTodayActive` and recalculates `streakDates`; `render` uses atomic `_state.update` and falls back to snapshot streak on initial load.
- Extracted `DayNumberText` in `CalendarScreen.kt` to centralize date semantics and eliminate duplicated text blocks across the 7 visual cell branches.
- Verification passed: `.\gradlew.bat :feature:calendar:impl:testDebugUnitTest :feature:calendar:impl:compileDebugAndroidTestKotlin :feature:calendar:impl:lintDebug` — BUILD SUCCESSFUL (10/10 unit tests, clean lint).
