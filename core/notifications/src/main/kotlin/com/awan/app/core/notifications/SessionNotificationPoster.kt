...
class SessionNotificationPoster @Inject constructor(
    ...
    fun post(event: SessionNotificationEvent, now: LocalDateTime) {
        ...
        when (event) {
            ...
            is SessionNotificationEvent.Ended -> postEnded(event.session)
        }
    }
    ...
    private fun postReminder(session: UpcomingSession, now: LocalDateTime) {
        ...
        notify(
            id = NotificationIds.reminder(session.id),
    ...
    private fun postEnded(session: UpcomingSession) {
        ...
        notify(
            id = NotificationIds.ended(session.id),
            builder = baseBuilder(AwanNotificationChannels.SESSION_END, session)
                .setContentTitle(context.getString(R.string.notifications_session_ended_title, session.title))
                .setContentText(text)
                .setAutoCancel(true)
                .setPriority(NotificationCompat.PRIORITY_HIGH),
            toastTitle = context.getString(R.string.notifications_session_ended_title, session.title),
            toastMessage = text,
        )
    }
